...
function renderTxTable(txList) {
  ...
  // Update Summary Bar
  const totalsDiv = document.getElementById('historyTotals');
  if (totalsDiv) {
    let totalCDF = 0, totalUSD = 0;
    txList.forEach(tx => {
      // For history perspective, credit adds, debit subtracts for total volume, or we just sum successful transactions?
      // Usually users want to know their net volume, or just total incoming?
      // Since it's all roles, let's just sum up successful 'credit' and 'debit' differently, or just 'succès' amounts?
      // Actually, just sum all successful credits (received) minus debits (sent), or just show them separately?
      // Let's show: "Revenus: xxx | Dépenses: xxx"
      if (tx.statut === 'succès') {
        const amt = Number(tx.montant) || 0;
        if (tx.currency === 'USD') {
          if (tx.action === 'credit') totalUSD += amt;
          else if (tx.action === 'debit') totalUSD -= amt;
        } else {
          if (tx.action === 'credit') totalCDF += amt;
          else if (tx.action === 'debit') totalCDF -= amt;
        }
      }
    });
    totalsDiv.innerHTML = `
      <span class="badge" style="background: rgba(16, 185, 129, 0.1); color: #10B981;">Solde (Filtré): ${window.formatMoney(totalCDF, 'CDF')}</span>
      <span class="badge" style="background: rgba(16, 185, 129, 0.1); color: #10B981;">${window.formatMoney(totalUSD, 'USD')}</span>
    `;
  }
}
