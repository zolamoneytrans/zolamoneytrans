...
window.showAdminTab = function(tabName) {
  ...
  const titleMap = {
    ...
    foreign: 'Comptes — Utilisateurs Internationaux',
  ...
  const viewIds = ['viewOverview', 'viewTransactions', 'viewAgents', 'viewUsers', 'viewForeign', 'viewMerchants', 'viewChurches', 'viewPayroll', 'viewKYC', 'viewSupport', 'viewSecurity', 'viewBroadcasts', 'viewLoyalty', 'viewSettings', 'viewTests', 'viewBendabus', 'viewGeneralSimulated', 'viewSuspectes'];
  viewIds.forEach(id => {
...
async function loadUsers() {
  try {
    ...
    if (typeof window.updateMarketingCounts === 'function') window.updateMarketingCounts();
...
// --- Foreign Users Logistics ---
window.renderForeignList = function(users) {
  const listEl = document.getElementById('foreignList');
  if (!listEl) return;
  listEl.innerHTML = '';
  
  const foreignUsers = users.filter(u => u.telephone && !u.telephone.startsWith('+243') && !u.telephone.startsWith('243') && !u.telephone.startsWith('00243'));
  
  if (foreignUsers.length === 0) {
    listEl.innerHTML = '<div style="padding:20px; text-align:center; color:var(--admin-text-muted);">Aucun client international trouvé</div>';
    return;
  }
  ...
  foreignUsers.forEach(u => {
    ...
    `;
    div.onclick = () => window.selectForeignUser(u);
    listEl.appendChild(div);
  });
};

window.filterForeignList = function(val) {
  const foreignUsers = allUsers.filter(u => u.telephone && !u.telephone.startsWith('+243') && !u.telephone.startsWith('243') && !u.telephone.startsWith('00243'));
  const filtered = foreignUsers.filter(u => 
    (u.nom && u.nom.toLowerCase().includes(val.toLowerCase())) || 
    (u.email && u.email.toLowerCase().includes(val.toLowerCase())) ||
    (u.telephone && u.telephone.includes(val))
  );
  // Directly render filtered to avoid double filter
  const listEl = document.getElementById('foreignList');
  if (!listEl) return;
  listEl.innerHTML = '';
...
window.selectForeignUser = function(user) {
  selectedUser = user;
  window.renderForeignList(allUsers); 
  document.getElementById('noForeignSelected').style.display = 'none';
  const panel = document.getElementById('foreignDetailPanel');
  ...
  if (user.kycDocuments && Object.keys(user.kycDocuments).length > 0) {
    Object.entries(user.kycDocuments).forEach(([key, url]) => {
      if(key !== 'selfie' && url) {
        kycDocsHtml += `
          <div style="background:var(--admin-bg-lighter); padding:10px; border-radius:8px; margin-bottom:10px; display:flex; align-items:center; gap:12px;">
            <div style="width:40px; height:40px; background:var(--c-purple); border-radius:6px; display:flex; align-items:center; justify-content:center;">📄</div>
            <div style="flex:1;">
              <div style="font-size:0.85rem; font-weight:600; text-transform:capitalize; color:#fff;">Document: ${key}</div>
            </div>
            <a href="${url}" target="_blank" class="btn-premium primary btn-sm" style="text-decoration:none;">Voir</a>
          </div>
        `;
      }
    });
  } else {
    kycDocsHtml = '<p style="color:var(--admin-text-muted); font-size:0.85rem;">Aucun document fourni.</p>';
  }

  panel.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:24px;">
      <div style="display:flex; gap:20px; align-items:center;">
        <div style="width:70px; height:70px; border:3px solid ${isVerified ? 'var(--c-success)' : 'var(--admin-border)'}; border-radius:50%; padding:2px;">
          ${avatarHtml}
        </div>
        <div>
          <h2 style="margin:0; font-size:1.4rem; display:flex; align-items:center; gap:8px;">
            ${user.nom || user.prenom || 'Anonyme'}
            ${isVerified ? '<span style="color:var(--c-success); font-size:1.2rem;" title="KYC Validé">✅</span>' : ''}
            ${isBlocked ? '<span style="color:var(--c-danger); font-size:1.2rem;" title="Compte Bloqué">🚫</span>' : ''}
          </h2>
          <div style="color:var(--c-gold); font-weight:600; font-size:0.9rem; margin-top:4px;">UID: ${user.id}</div>
        </div>
      </div>
      <div style="text-align:right;">
        <div style="color:var(--admin-text-muted); font-size:0.85rem;">Solde Actuel</div>
        <div style="font-size:1.8rem; font-weight:800; color:var(--c-gold);">${window.fmtCDF(user.balance || 0)}</div>
        <div style="font-size:1.1rem; color:#fff;">${window.fmtUSD(user.soldeUSD || 0)}</div>
      </div>
    </div>
    
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-bottom:24px;">
      <div style="background:var(--admin-bg-lighter); padding:16px; border-radius:10px; border:1px solid var(--admin-border);">
        <h3 style="font-size:0.9rem; color:var(--admin-text-muted); margin-bottom:12px; text-transform:uppercase; letter-spacing:1px;">Contact & Info</h3>
        <div style="display:flex; flex-direction:column; gap:8px; font-size:0.9rem;">
          <div style="display:flex; justify-content:space-between;">
            <span style="color:var(--admin-text-muted);">E-mail:</span>
            <span style="font-weight:600;">${user.email || 'N/A'}</span>
          </div>
          <div style="display:flex; justify-content:space-between;">
            <span style="color:var(--admin-text-muted);">Téléphone:</span>
            <span style="font-weight:600;">${user.telephone || 'N/A'}</span>
          </div>
          <div style="display:flex; justify-content:space-between;">
            <span style="color:var(--admin-text-muted);">Pays:</span>
            <span style="font-weight:600;">${user.pays || 'International'}</span>
          </div>
          <div style="display:flex; justify-content:space-between;">
            <span style="color:var(--admin-text-muted);">Statut KYC:</span>
            <span style="font-weight:600; color:${isVerified ? 'var(--c-success)' : 'var(--c-warning)'};">${user.kycStatus || 'Non soumis'}</span>
          </div>
        </div>
      </div>
      
      <div style="background:var(--admin-bg-lighter); padding:16px; border-radius:10px; border:1px solid var(--admin-border);">
        <h3 style="font-size:0.9rem; color:var(--admin-text-muted); margin-bottom:12px; text-transform:uppercase; letter-spacing:1px;">Documents (KYC)</h3>
    ...
    <div style="display:flex; gap:12px; margin-top:20px; border-top:1px solid var(--admin-border); padding-top:20px;">
      <button class="btn-premium primary" onclick="window.openAdminEmailModal('${user.email || ''}', '${user.nom || ''}', 'user')">
        ✉️ Contacter
      </button>
    </div>
  `;
...
window.downloadForeignUsersCSV = function() {
  const foreignUsers = allUsers.filter(u => u.telephone && !u.telephone.startsWith('+243') && !u.telephone.startsWith('243') && !u.telephone.startsWith('00243'));
  if (!foreignUsers || foreignUsers.length === 0) return alert('Aucun utilisateur international à exporter');
  const headers = ["ID", "Nom", "Email", "Téléphone", "Pays", "Solde_CDF", "Solde_USD", "KYC_Status"];
  const rows = foreignUsers.map(u => [
    u.id, u.nom || u.prenom || '', u.email || '', u.telephone || '', u.pays || 'International', u.balance || 0, u.soldeUSD || 0, u.kycStatus || 'non-soumis'
  ]);
  ...
  link.setAttribute("download", "zola_international_users.csv");
...
function renderOverviewTxTable(txs) {
  ...
  txs.forEach(tx => {
    ...
    const currency = (tx.devise || tx.currency || 'USD').toUpperCase();
    const finalCurrency = (currency !== 'USD' && currency !== 'CDF') ? 'USD' : currency;
    ...
    tr.innerHTML = `
      ...
      <td style="font-weight:700; color:#fff;">${tx.montant} ${finalCurrency}</td>
...
function initDRCMapCanvas() {
  ...
  function drawMap() {
    ...
    cities.forEach(city => {
      ...
      city.pulse += 0.05;
      const rippleSize = Math.max(0, 8 + Math.sin(city.pulse) * 12);
