...
function buildDestChart(mobileCount, bankCount, zolaCount) {
  ...
  destChartInstance = new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: ['Mobile Money', 'Compte Bancaire', 'Compte Zola'],
      datasets: [{
        data: [mobileCount, bankCount, zolaCount],
...
onAuthStateChanged(auth, async user => {
  ...
  // Volume mensuel et destinations (live)
  onSnapshot(
    ...
    snap => {
      let volCDF = 0, volUSD = 0;
      let mobileCount = 0, bankCount = 0, zolaCount = 0;
      ...
      snap.forEach(d => {
        ...
        const op = (tx.operateur || '').toLowerCase();
        if (op.includes('banque') || op.includes('bank')) {
          bankCount++;
        } else if (op.includes('zola')) {
          zolaCount++;
        } else {
          mobileCount++;
        }
      ...
      buildDestChart(mobileCount, bankCount, zolaCount);
    }
  );

  // Paiements en attente (Validations)
  onSnapshot(
    query(collection(db,'transactions'), where('userId','==',user.uid), where('statut','==','en_attente')),
    snap => {
  ...
  // Employés inscrits (simulé depuis la sous-collection employees)
  onSnapshot(
    collection(db, 'users', user.uid, 'employees'),
    snap => {
      const el = document.getElementById('statEmployees');
      if (el) el.textContent = snap.size;
    },
    err => {
      // Si la collection n'existe pas ou règles bloquent, afficher 0
      const el = document.getElementById('statEmployees');
